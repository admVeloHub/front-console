import { API_CONFIG, getApiKey, getHeaders, GPT_PROMPTS } from '../config/api';

// Interface para a resposta da API do GPT
...
// Interface para dados da avaliação
export interface AvaliacaoData {
  id: string;
  colaboradorNome: string;
  arquivoLigacao?: string;
  nomeArquivo?: string;
}
...
class GPTService {
  ...
  public async analyzeCall(avaliacao: AvaliacaoData): Promise<GPTResponse> {
    if (!this.isConfigured) {
  ...
  private async callOpenAIAPI(avaliacao: AvaliacaoData): Promise<GPTResponse> {
    ...
    try {
      ...
      const response = await fetch(`${API_CONFIG.OPENAI.BASE_URL}/chat/completions`, {
        method: 'POST',
        headers: getHeaders('openai') as Record<string, string>,
        body: JSON.stringify({
          model: API_CONFIG.OPENAI.MODEL,
          messages: [
            {
  ...
  private buildPrompt(avaliacao: AvaliacaoData): string {
...
Por favor, forneça uma análise completa seguindo exatamente o formato JSON especificado nas instruções.`;
  ...
  private validateAndTransformResponse(response: any): GPTResponse {
  ...
  private async fallbackAnalysis(avaliacao: AvaliacaoData): Promise<GPTResponse> {
...
*Relatório gerado em modo fallback - Configure a API OpenAI para análises mais precisas*`;
  ...
  public getUsageStats(): { totalAnalyses: number; apiCalls: number; fallbackUses: number } {
    return {
...
export const analyzeCallWithGPT = (avaliacao: AvaliacaoData) => gptService.analyzeCall(avaliacao);
export const configureGPT = (apiKey: string) => gptService.configure(apiKey);
